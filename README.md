...
Google Timeline already knows where I went, how long I stayed, and how I moved. I wanted to press Play. There I Was imports a Timeline JSON export, finds trips between returns home, names the places that mattered, and rebuilds each trip as a route you can replay. GPT-5.6 chooses the replay chapters, pacing, and grounded captions. The finished route becomes a memory card you can keep or share.
...
- Uses GPT-5.6 to direct replay pacing and on-map captions from named places, dates, travel modes, and summarized movement.
- Turns the completed replay into a 1080 x 1350 PNG containing the route, edited memory line, trip facts, and optional photos.
...
5. MapLibre renders the full loop over OpenFreeMap tiles. One animation clock controls the marker, route progress, timestamp, photo moments, and GPT-directed chapters.
6. The selected trip becomes a coordinate-free Memory Dossier. A Netlify Function validates it, calls the OpenAI Responses API with `store: false`, and validates the structured direction again.
7. Canvas renders the edited plan and route locally as a downloadable memory card.

More detail is in [docs/architecture.md](docs/architecture.md).

## GPT-5.6 Memory Director

The model receives one bounded dossier. The original export stays in the browser. The dossier can include named destinations, dates, duration, modes, coverage gaps, and answers the user typed. GPT-5.6 returns three to five chronological chapters plus timed captions. Those timestamps control the map replay, and the captions appear on the route as it plays. The prompt forbids invented activities, companions, purpose, emotion, weather, and events. Every response must match a strict Zod schema.

Provider errors return a deterministic direction from the same named evidence. The live production endpoint is configured for `gpt-5.6`.

## Memory card

The edited direction becomes a 4:5 PNG rendered in the browser. It contains the trip dates, final home-to-home loop, destination markers, memory line, chapter titles, miles, nights, places, and up to three uploaded photos. Supported devices can share the file directly; every browser can download it.
...
- **Delete imported data** clears trips, route cache, prior directions, and photos from IndexedDB.
...
The committed sample test requires exactly three trips with the expected titles and dates. It also generates a directed replay, displays a grounded map caption, and downloads the California memory card. The private acceptance suite imports 15,989 Timeline records, verifies 28 unique named trips, opens the California replay, and checks playback. Unit coverage includes parsing, home inference, trip boundaries, route selection, loop closure, directed pacing, memory-card geometry, photo metadata, dossier minimization, and structured output validation.
