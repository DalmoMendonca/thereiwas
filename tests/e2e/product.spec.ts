...
test('real sample data detects exactly the requested three trips', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Direct replay' }).click()
  await expect(page.getByRole('heading', { name: 'Directed replay' })).toBeVisible()
  await expect(page.locator('.memory-card-preview img')).toBeVisible({ timeout: 65_000 })
  await page.getByRole('button', { name: 'Play memory' }).click()
  await expect(page.locator('.map-story-caption')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Pause replay' })).toBeVisible()
  await page.getByRole('button', { name: 'Pause replay' }).click()
  const downloadPromise = page.waitForEvent('download')
  await page.getByRole('button', { name: 'Save PNG' }).click()
  await expect((await downloadPromise).suggestedFilename()).toBe('there-i-was-california.png')
})
...
test('private Timeline acceptance: named, unique trips and a complete route replay', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Your trips' })).toBeVisible({ timeout: 60_000 })
