...
> **Product reset, July 20, 2026:** Direct user testing supersedes every contradictory requirement below. The landing page is a no-scroll screen with `Import Timeline JSON`, `Try with Sample Data`, the exact Google Maps export path, and the `Build with ❤️ | dalmo.ai` footer. The sample is a pared-down excerpt of the supplied Timeline containing exactly the latest California, New York, and Italy trips. Trip rows use geographic titles, chronological significant-city trails, and numeric dates. The trip page centers a tightly fitted home-to-home loop with persistent road-following geometry from Mapbox Directions where Timeline evidence is sparse, miles in a small side rail, no finale overlay, and no point-by-point journey thread. Optional photo upload reads EXIF time and GPS locally, adds map markers and a filmstrip, and synchronizes photos with replay. GPT-5.6 must work in production from a server-side key, receive resolved city, region, country, date, mode, and summarized-leg evidence, and never produce "Unnamed stop." Its chapters and timed captions direct the map replay rather than living in a secondary story panel. A completed replay must resolve into a locally rendered, shareable 1080 by 1350 memory card containing the route, trip summary, and grounded chapters or photos. Update the Devpost draft and production app after validation, then create a new explainer video under three minutes.
